...
# Estilização Técnica (Branding Felipe Alves Consultoria)
st.markdown("""
    <style>
...
if 'db_ativos' not in st.session_state:
    st.session_state['db_ativos'] = pd.DataFrame([
        {"Tag": "TA-001", "Tipo": "Vaso de Pressão", "Categoria": "V", "Fluido": "Ar Comprimido", "Classe": "C", "Status": "Em Dia", "Vencimento": "2025-10-15"},
        {"Tag": "VC-102", "Tipo": "Vaso de Pressão", "Categoria": "II", "Fluido": "Amônia", "Classe": "A", "Status": "Vencido", "Vencimento": "2025-06-20"}
    ])

if 'db_instrumentos' not in st.session_state:
    st.session_state['db_instrumentos'] = pd.DataFrame([
        {"Tag_Inst": "PSV-01", "Equipamento": "TA-001", "Vencimento": "2024-08-22", "Status": "Vencido"}
    ])
...
with st.sidebar:
    ...
    with st.expander("Dados da Unidade", expanded=False):
        ...
        resp_cl = st.text_input("Engenheiro Responsável", "Eng. Felipe Alves")
    st.divider()
...
with col_t:
    ...
    st.caption(f"Unidade: {setor_cl} | Data do Sistema: 22/03/2026")
with col_c:
    st.markdown("""
